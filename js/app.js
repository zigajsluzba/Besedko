import { Game } from "./game.js?v=20260626-2";
import { Dictionary } from "./dictionary.js?v=20260626-2";
import { Storage } from "./storage.js?v=20260626-2";
import { UI } from "./ui.js?v=20260626-2";
...
async function init() {
  try {
    ...
    const storedMode = storage.get("mode", window.localStorage.getItem("besedko-mode") || "single");
    ...
    if (mode === "multiplayer") {
      ...
      if (!restored) {
        ui.setMultiplayerStatus("Multiplayer je pripravljen. Ustvari sobo ali se pridruži.");
      }
    }
    ui.setGame(game);
    ui.setMode(mode);
    ...
    window.__besedkoInitError = error && (error.stack || error.message || String(error));
